...
use Rack::Recaptcha, :public_key => RECAPTCHA_PUB_KEY, :private_key => RECAPTCHA_PRIV_KEY
helpers Rack::Recaptcha::Helpers
Rack::Recaptcha.test_mode! if ENVIRONMENT == :development
$stdout.sync = true
...
Thread.new do
  while true do
    ...
    if s1 != IPDPicture.clients.size
      ...
      LOG_HANDLE.info("REMOVE STALE CLIENTS #{s1 - IPDPicture.clients.size}/#{s1}")
    end
  end
end
...
helpers do
  def protected!
    unless authorized?
      ...
      @msg = "Not authorized."
      halt [401, slim(:notice, :pretty => RENDER_PRETTY)]
    end
  end

  def authorized?
    @auth ||=  Rack::Auth::Basic::Request.new(request.env)
    @auth.provided? && @auth.basic? && @auth.credentials && @auth.credentials == [HTTP_AUTH_USER, HTTP_AUTH_PASS]
  end
end
...
get '/picture/delete/:filename' do
  ...
  unless params[:filename] =~ /^\d+\.(\d+\.)?[a-z]{3,4}$/i
    ...
    halt slim :notice, :pretty => RENDER_PRETTY
  end
  # check if picture exists
  # TODO
  # use IPDPicture.exists?(params[:filename]) ?
  result = DB_HANDLE.execute("SELECT id FROM picture WHERE filename = ?", [params[:filename]])
  if result.empty?
    # TODO
  ...
  LOG_HANDLE.info("DELETE PICTURE ID #{result[0][0]} / #{request.ip} / #{request.user_agent}")
  @msg = "Picture deleted."
  halt slim :notice, :pretty => RENDER_PRETTY
end
...
get '/picture/show/detail/:filename' do
  ...
  @picture.dump = "detail"
  slim :picture, :pretty => RENDER_PRETTY, :layout => false
end
...
get '/picture/show/user/:nick' do
  ...
  if request.has_dump? and IPDUser.exists?(params[:nick])
	...
	rnd_picture = DB_HANDLE.execute("SELECT p.id, p.filename, p.time_taken, p.time_sent, p.id_user, p.path, p.precursor, p.successor, u.nick, u.accept_external_messages, p.no_show FROM picture p JOIN user u ON p.id_user = u.id WHERE p.id = ?", [random_id])
	err = false
	if rnd_picture.empty?
	  err = true
	  LOG_HANDLE.warn("PICTURE MISSING WARNING ID #{random_id} DUMP #{id_dump}")
	elsif rnd_picture[0][10] == 1
	  err = true
	  LOG_HANDLE.warn("PICTURE NO_SHOW WARNING ID #{random_id} DUMP #{id_dump}")
	elsif !File.exists?(PICTURE_DIR + "/" + rnd_picture[0][5] + "/" + rnd_picture[0][1])
	  err = true
	  LOG_HANDLE.error("PICTURE MISSING ERROR #{rnd_picture[0][5]}/#{rnd_picture[0][1]}")
	end
	if err
	  i += 1
	...
	break
      ...
      LOG_HANDLE.fatal(e.message) if e.class.name == "IPDPictureError"
      @msg = "No pictures."
      halt slim :notice, :pretty => RENDER_PRETTY
    end
    ...
    slim :dump, :pretty => RENDER_PRETTY, :layout => false
  else
...
get '/picture/contact/:nick/about/:filename/in/:dump' do
  ...
  @msg = ""
  @message = ""
  slim :contact, :pretty => RENDER_PRETTY
end
...
post '/picture/contact/:nick/about/:filename/in/:dump' do
  ...
  if params[:message].nil? or params[:message].empty? or params[:message] =~ /\A\s+\z/ or !recaptcha_valid?
    ...
    halt slim :contact, :pretty => RENDER_PRETTY
  end
  message = params[:message].gsub(/\r/, "").strip
  message.gsub!(/(#{REGEX_EMAIL})/i, '<a href="mailto:\1?subject=Your fan mail">\1</a>')
  ...
  slim :notice, :pretty => RENDER_PRETTY
end
...
get '/user/show/:nick' do
  ...
  posts = DB_HANDLE.execute("SELECT COUNT(*) FROM picture WHERE id_user = ?", [@user.id])
  @user.posts = posts[0][0]
  messages = DB_HANDLE.execute("SELECT * FROM message WHERE id_user = ? AND time_created >= ? ORDER BY time_created DESC", [@user.id, Time.now.to_i - MSG_SHOW_SPAN])
  @msgs = []
  messages.each do |row|
    @msg = IPDMessage.new
    @msg.id = row[0]
    @msg.message_id = row[1]
    @msg.message_text = MSG[row[1]]
    @msg.time_created = row[2]
    @msg.id_user = @user.id
    @msgs.push(@msg)
  end
  slim :user, :pretty => RENDER_PRETTY
end
...
get '/dump/check/password/:dump' do
  ...
  slim :check_password, :pretty => RENDER_PRETTY
end
...
post '/dump/check/password/:dump' do
  ...
  if @dump.password != password
    @msg = "Wrong keyword."
    halt slim :check_password, :pretty => RENDER_PRETTY
  end
  response.set_cookie(@dump.id, :value => Digest::RMD160::hexdigest(@dump.time_created.to_s + password.to_s), :path => "/#{@dump.alias}", :expires => Time.now + COOKIE_LIFETIME)
  redirect "/#{@dump.alias}"
end

##############################
get '/dump/list/?' do
  result = DB_HANDLE.execute("SELECT d.id FROM dump d JOIN picture p ON d.id = p.id_dump AND p.id NOT NULL WHERE d.state != ? GROUP BY d.id ORDER BY d.alias ASC;", ["hidden"])
  unless result.any?
    @msg = "No dumps yet."
    halt slim :notice, :pretty => RENDER_PRETTY
  end
  ...
  slim :dump_list, :pretty => RENDER_PRETTY
end
...
get '/dump/show/:dump' do
  ...
  result = DB_HANDLE.execute("SELECT COUNT(*) FROM \"#{@dump.id}\"")
  @picture_counter = result[0][0]
  result = DB_HANDLE.execute("SELECT COUNT(*) FROM mapping_dump_user WHERE id_dump = ?", [@dump.id])
  @user_counter = result[0][0]
  result = DB_HANDLE.execute("SELECT id_user, count(1) posts FROM \"#{@dump.id}\" GROUP BY id_user ORDER BY posts DESC, id_user ASC LIMIT 10")
  @users = []
  result.each do |row|
    user = IPDUser.load(row[0])
    unless user
      LOG_HANDLE.error("USER MISSING ERROR ID #{row[0]} / #{caller[0]}")
      next
    end
    user.posts = row[1]
    @users.push(user)
  ...
  (0..@users.length - 1).each do |i|
    if DUMP_HONOR_TITLES[i]
      if DUMP_HONOR_TITLES[i] =~ /%i(\"[^"]+\")/
	nick = @users[i].nick.split(" ")
	nick.insert(1, $1)
	nick_w_title = nick.join(" ")
      else
	nick_w_title = DUMP_HONOR_TITLES[i].sub(/%n/, @users[i].nick)
  ...
  slim :dump_show, :pretty => RENDER_PRETTY
end

##############################
get '/dump/create/?' do
  @msg = ""
  slim :dump_create, :pretty => RENDER_PRETTY
end
...
post '/dump/create/?' do
  ...
  if dump_alias.length > 64
    ...
    halt slim :dump_create, :pretty => RENDER_PRETTY
  end
  result = DB_HANDLE.execute("SELECT * FROM dump WHERE alias = ?", [dump_alias])
  if result.any?
    @msg = "Sry, this dump already exists."
    halt slim :dump_create, :pretty => RENDER_PRETTY
  end
  reserved = File.readlines(RESERVED_DUMPS)
  # \n chomp hack
  ...
  if user
    ...
    dump.save
    LOG_HANDLE.info("NEW DUMP #{dump.alias} / #{request.ip} / #{request.user_agent}")
...
get '/admin/pool/empty' do
  ...
  LOG_HANDLE.info("POOL EMPTIED #{request.ip} / #{request.user_agent}")
  IPDPicture.random_pool.to_json
end

##############################
get '/' do
  slim :landing, :pretty => RENDER_PRETTY
end

##############################
get '/about.html' do
  @counter = IPDPicture.count_pictures
  slim :about, :pretty => RENDER_PRETTY
end
...
get '/*' do
  if request.has_dump? and IPDDump.dump.has_key?(request.dump)
    ...
    begin
      ...
      while true
	random_id = IPDPicture.get_smart_random_id(request)
	rnd_picture = DB_HANDLE.execute("SELECT p.id, p.filename, p.time_taken, p.time_sent, p.id_user, p.path, p.precursor, p.successor, u.nick, u.accept_external_messages, d.alias, p.no_show FROM picture p JOIN user u ON p.id_user = u.id JOIN dump d ON p.id_dump = d.id WHERE p.id = ?", [random_id])
	err = false
	if rnd_picture.empty?
	  err = true
	  LOG_HANDLE.warn("PICTURE MISSING WARNING ID #{random_id} DUMP #{id_dump}")
