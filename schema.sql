CREATE TABLE "picture" (id integer primary key autoincrement, filename text, time_taken int, time_sent int, id_user int, original_hash text, id_dump int, path text, precursor int, successor int);
sqlite> .schema
...
CREATE TABLE mapping_dump_user(id_dump int, id_user int);
